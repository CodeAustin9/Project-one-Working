...
function initApp() {
  ...
  firebase.auth().onAuthStateChanged(function(user) {
    ...
    document.getElementById('btnVerify-email').disabled = true;
    // [END_EXCLUDE]
    if (user) {
      // User is signed in.
      var email = user.email;
      var uid = user.uid;
      window.location = 'myRecipe.html';
      

      // [START_EXCLUDE]
      document.getElementById('btnLogin-status').textContent ='Signed in';
      document.getElementById('btnLogout').textContent ='Sign out';
      if (!emailVerified) {
        document.getElementById('btnVerify-email').disabled = false;
      }
      // [END_EXCLUDE]
    } else {
      // User is signed out.
  ...
  document.getElementById('btnVerify-email').addEventListener('click', sendEmailVerification, false);
  document.getElementById('btnPassword-reset').addEventListener('click', sendPasswordReset, false);
}
